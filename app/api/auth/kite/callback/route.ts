...
export async function GET(request: NextRequest) {
  ...
  if (status !== 'success' || !requestToken) {
    return NextResponse.redirect(new URL('/home?kite_auth=failed', request.url));
  }

  try {
    const session = await getSession();
    if (!session?.userId) {
      return NextResponse.redirect(new URL('/sign-in?kite_auth=failed', request.url));
    }
    ...
    return NextResponse.redirect(new URL('/home?kite_auth=success', request.url));
  } catch (err) {
