...
import { parseCSV, parseXLSX, type ImportRow } from '@/lib/portfolio-io';
...
function ModalContent({ onClose, onImported }: { onClose: () => void; onImported?: (count: number) => void }) {
  ...
  async function handleImport() {
    ...
    setSubmitting(false);
    if (failed.length > 0) {
      setImportErrors([`Failed to import: ${failed.join(', ')}`]);
    }
    setStep('done');
          ...
          <div className="flex items-center justify-between px-8 py-6 border-b border-[#2f3445]/60">
            <div>
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#8c909f] mt-0.5">
                CSV or Excel · auto-logged to Activity
              </p>
            </div>
                ...
                <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      dragging ? 'border-[#4d8eff] bg-[#4d8eff]/10' : 'border-[#2f3445] hover:border-[#424754] hover:bg-[#1a2035]/50',
                    )}
                  >
                    ...
                    <p className="text-xs text-[#8c909f]">or click to browse · CSV, XLS, XLSX supported</p>
                    <input ref={fileRef} type="file" accept=".csv,.xls,.xlsx,.txt" className="hidden" onChange={handleFileChange} />
                  </div>

                  {/* Template hint */}
                  <div className="mt-5 p-4 rounded-xl" style={fieldStyle}>
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#8c909f] mb-2">Expected Columns</p>
                    <p className="text-[11px] text-[#424754] font-mono leading-relaxed">
                      Ticker, Name, Units, Price, Date, Action, Sector, Broker, Asset Type
                    </p>
                    <p className="text-[10px] text-[#424754] mt-2">
                      Only <span className="text-[#adc6ff]">Ticker</span>, <span className="text-[#adc6ff]">Units</span> and <span className="text-[#adc6ff]">Price</span> are required. Action defaults to <em>Buy</em>.
                    </p>
                  </div>
                  ...
                  {errors.length > 0 && (
                    <div className="mt-4 p-4 rounded-xl bg-[#ffb2b7]/10 border border-[#ffb2b7]/20">
                      {errors.map((e, i) => (
                        <p key={i} className="text-xs text-[#ffb2b7] font-semibold">{e}</p>
                      ))}
                    </div>
                  )}
                </motion.div>
              ...
              {step === 'preview' && (
                <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => { setStep('upload'); setRows([]); setErrors([]); setFileName(''); }} className="text-[10px] font-black uppercase tracking-widest text-[#8c909f] hover:text-[#dde2f8] transition-colors">
                      Change file
                    </button>
                  </div>
                  ...
                  {errors.length > 0 && (
                    <div className="p-3 rounded-lg bg-[#D4AF37]/10 border border-[#D4AF37]/20">
                      <p className="text-[10px] font-black uppercase tracking-widest text-[#D4AF37] mb-1">Skipped rows</p>
                      {errors.slice(0, 5).map((e, i) => <p key={i} className="text-[10px] text-[#D4AF37]/80">{e}</p>)}
                    </div>
                  )}
                            ...
                            <tr key={i} className={cn('border-b border-[#2f3445]/40', i % 2 === 1 && 'bg-[#141c30]/50')}>
                              ...
                              <td className="px-3 py-2">
                                <span className={cn(
                                  ...
                                  row.action === 'Buy' ? 'bg-[#4edea3]/15 text-[#4edea3]' : 'bg-[#ffb2b7]/15 text-[#ffb2b7]',
                                )}>{row.action}</span>
                              </td>
                  ...
                  <div className="flex gap-3 pt-1">
                    <button onClick={onClose} className="flex-1 h-12 rounded-lg text-[10px] font-black uppercase tracking-widest text-[#8c909f] hover:text-[#dde2f8] transition-colors" style={fieldStyle}>
                      Cancel
                    </button>
                    <button
...
export function ImportModal({ open, onClose, onImported }: Props) {
  ...
  return createPortal(
    <AnimatePresence>
      {open && <ModalContent onClose={onClose} onImported={onImported} />}
    </AnimatePresence>,
    document.body,
  );
}
